...
```bash
kinesis-consumer$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
kinesis-consumer$ sam local start-api
kinesis-consumer$ curl http://localhost:3000/
```
...
```bash
kinesis-consumer$ sam logs -n HelloWorldFunction --stack-name kinesis-consumer --tail
```
